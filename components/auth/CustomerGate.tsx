...
/** Client-side guard for customer-only screens (plans, compare). */
export function CustomerGate({ children }: Props) {
    const router = useRouter();
    const { user, bootstrapping } = useAuth();

    useEffect(() => {
        if (!bootstrapping && user && !isCustomer(user)) {
            router.replace('/(app)');
        }
    ...
    if (!isCustomer(user)) {
        return (
            <View style={styles.center}>
                ...
                <Text style={styles.hint}>This area is for customer accounts.</Text>
            </View>
        );
    }
